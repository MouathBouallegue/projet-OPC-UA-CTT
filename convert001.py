...
tree = ET.parse('Serv-Config.results.xml')  # Remplace avec ton chemin XML
root = tree.getroot()
...
# Liste des tests à inclure (laisser vide = inclure tous)
include_test_steps = set()  # ex: {"Read", "Write", "Browse"}

# Fonction pour extraire la 1re phrase utile
...
def get_reason(node):
    ...
    if error is not None:
        ...
        if desc.startswith(("ActivateSession", "Write()", "Read()")):
            return desc
...
# Extraire les résultats depuis le XML
def process_results(root):
    ...
    for result_node in root.findall(".//ResultNode"):
        ...
        if include_test_steps and name not in include_test_steps:
            continue
