/*  Test prepared by Ing.-Buero Allmendinger: info@allmendinger.de
    Description:
        Request Value attribute with dataEncoding = `Default XML`.

    Expected Results:
        Service result is `Good`. Operation level result is `Good`.
*/

function readEncoding002() {
    ...
    for ( var i = 0; itemsValue.length > i; i++ ) {     
        for ( var v = 0; BrowseHelper.Response.Results[i].References.length > v; v++ ) {
            if ( BrowseHelper.Response.Results[i].References[v].BrowseName.Name == "Default XML" ) {
                itemsValue[i].DataEncoding.Name = "Default XML";
                hasEncoding = true;
            }
            else {
                print( "Node: " + itemsValue[i].NodeId + "  doesn't support 'Default XML' Encoding." );
            }
        }
    }

    if ( hasEncoding ) {
        // perform a read using "Default XML" for the encoding
        return( ReadHelper.Execute( {
                    NodesToRead: itemsValue,
                    TimestampsToReturn: TimestampsToReturn.Server } ) );
    }
    else {
        addSkipped( "None of the Structures support Encoding in 'Default XML'." );
        return ( false );
    }
}
Test.Execute( { Procedure: readEncoding002 } );