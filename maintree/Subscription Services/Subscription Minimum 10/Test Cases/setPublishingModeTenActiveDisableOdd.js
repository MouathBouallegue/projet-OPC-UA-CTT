...
Test.Execute( { Debug: true, Procedure: function test() {
    ...
    if( SetPublishingModeHelper.Execute( { SubscriptionIds: oddSubscriptions, PublishingEnabled: false } ) ) {
        ...
        // check if the write response is GoodCompletesAsynchronously
        if( WriteHelper.Response.Results[0].StatusCode == StatusCode.GoodCompletesAsynchronously ) {
            // check if the Delay is greater than the KeepAlivePeriod
            if( CreateSubscriptionHelper.Response.RevisedPublishingInterval * CreateSubscriptionHelper.Response.RevisedMaxKeepAliveCount + CreateSubscriptionHelper.Response.RevisedPublishingInterval < Settings.ServerTest.Capabilities.GoodCompletesAsynchronouslyDelay ) {
                // delete all Subscriptions and skip
                for( i=0; i<subscriptions.length; i++ ) {
                ...
                addSkipped("The GoodCompletesAsynchronouslyDelay setting is greater than the KeepAlivePeriod. Skipping test!");
                return( true );
            }
        }
