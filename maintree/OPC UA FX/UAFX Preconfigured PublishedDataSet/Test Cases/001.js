/*  Test prepared by Ingenieurbuero Allmendinger: info@allmendinger.de
    Description : Verify that the server exposes the PublishSubscribe and PublishedDataSet
                  objects
    Requirements: At least one PreconfiguredPublishedDataSet exists, if not skip entire CU
            ...
            if( serverChildren[i].BrowseName.Name == "PublishSubscribe" ) {
                ...
                if( pubSubChildren.length > 0 ) {
                    for( var p=0; p<pubSubChildren.length; p++ ) {
                        if( pubSubChildren[p].BrowseName.Name == "PublishedDataSets" ) {
                            addLog( "'PublishedDataSets' object found in the 'PublishSubscribe' object of the server" );
                            break;
                        }
                        if( p == pubSubChildren.length - 1 ) {
                            addError( "Step 2: Could not find 'PublishedDataSets' object in the 'PublishSubscribe' object of the server." );
